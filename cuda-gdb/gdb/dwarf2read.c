  ...
  if (m_context)
    {
      ...
      if (dwarf_line_debug)
	printf_filtered("record_line: line %d address %lx inlined by %s\n",
			m_line, m_address, inline_function_name);
    }
#endif
...
cuda_decode_line_table (struct objfile *objfile)
{
  ...
  do
	...
	return;
      ...
      cu->reset_builder ();
      cu->start_symtab (objfile->original_name, "", 0);

      dwarf2_start_subfile (cu, objfile->original_name, "");
      ...
      for (int i = 0; i < cu->line_header->file_names.size (); i++)
	{
	  ...
	  /* fixup the global and static blocks */
