...
#define RECV_PACKET_BUFFER_SIZE 1024
...
static long inBufferSize = 0;
static uint32_t outBufferSize = 0;
static uint32_t outBufferUsed = 0;
...
static CUDBGResult
cudbgipcAppendRemote(void *d, uint32_t size)
{
...
{
    static gdb::char_vector rcvbuf(RECV_PACKET_BUFFER_SIZE);
    ...
    for (offset = 0, writeCount = 0; offset < out->dataSize; offset += writeCount) {
        ...
        if (writeCount < 0) {
            ...
            if (errno != EAGAIN && errno != EINTR) {
                cudbgipc_trace("Fifo write error (from=%u, to=%u, out->dataSize=%lu, offset=%lu, errno=%d)",
                               out->from, out->to, (unsigned long)out->dataSize, (unsigned long)offset, errno);
                return CUDBG_ERROR_COMMUNICATION_FAILURE;
            }
            writeCount = 0;
        }
...
cudbgipcRead(CUDBGIPC_t *in, void *buf, uint32_t size)
{
    ...
    for (offset = 0; offset < size; offset += readCount) {
        ...
        if (readCount < 0) {
            ...
            if (errno != EAGAIN && errno != EINTR) {
                cudbgipc_trace("Fifo read error (from=%u, to=%u, size=%u, offset=%lu, errno=%d)",
                               in->from, in->to, size, (unsigned long)offset, errno);
                return CUDBG_ERROR_COMMUNICATION_FAILURE;
            }
            readCount = 0;
        }
...
static CUDBGResult
cudbgipcAppendLocal(void *d, uint32_t size)
{
    CUDBGResult res;
    uint32_t dataSize = 0;
    void *data = NULL;
