...
cuda_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  ...
  /* 256 registers + 1 PC + 7 regular predicates + 64 uniform registers + 8 uniform predicates */
  tdep->num_regs           = 256 + 1 + 7 + 64 + 8;

  tdep->pc_regnum          = 256;

  tdep->first_pred_regnum  = tdep->pc_regnum + 1;
  tdep->last_pred_regnum   = tdep->first_pred_regnum + 6;
