...
#endif
{
  ...
  e->pc = pc;
#ifdef CUDA_DEBUG_LINE_EXTENSION
  if (context && (context < subfile->line_vector->nitems))
    {
      struct linetable_entry *ctx = &subfile->line_vector->item[context - 1];

      struct cuda_debug_inline_info *inline_info =
	(struct cuda_debug_inline_info *)
	xmalloc (sizeof (struct cuda_debug_inline_info));

      inline_info->line = ctx->line;
      inline_info->filename = subfile->name;
